...
func GetInstance() (*Config, error) {
	once.Do(func() {
		configInstance = &Config{}
		flags := ParseFlags()
		onceErr = ParseFile(flags.ConfigPath, configInstance, flags.Template)
	})
	return configInstance, onceErr
}
...
// ParseFile ...
func ParseFile(path string, pb *Config, template bool) error {
...
func loadFromEnv(pb *Config) error {
...
func parseYAML(contents []byte, pb *Config) error {
	// Decode YAML.
	...
	if err := json.Unmarshal(jsonBuffer.Bytes(), pb); err != nil {
		return err
	}
